...
namespace MixVerse.Game.Kart
{
    public sealed class KartExplosionDebrisView : MonoBehaviour
    {
        ...
        {
            var random = new System.Random(unchecked(seed * 7919 ^ System.Environment.TickCount));
            for (var i = 0; i < _chunks.Length; i++)
            {
                ...
                var size = i < 5 ? 1.25f + (float)random.NextDouble() * 0.95f
                    : i < 13 ? 0.55f + (float)random.NextDouble() * 0.75f
                    : 0.15f + (float)random.NextDouble() * 0.45f;
        ...
        {
            var fade = Mathf.Clamp01((VisibleSeconds - age) / 0.7f);
            for (var i = 0; i < _chunks.Length; i++)
            {
            ...
            gameObject.SetActive(age < VisibleSeconds);
        }
    }
}
